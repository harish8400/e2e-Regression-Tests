...
test("RTR is processed with TFN -@TFN_RTR", async ({ memberPage, superSteam, globalPage, apiRequestContext }) => {
    ...
    await test.step("Generate XML file for upload", async () => {
        ...
        //Here we have set first parameter as isMemberToSelectExsisting which is passing from JSON, so if we make it as true will select Exsisting Member or else if it will create a New Member from API and perform remaining process
        //Here we have set second parameter as isTFNToBePassed which is passing from JSON, so if we make it as false it will provide a TFN for the New Member created from API If we set to true it will create a new member without TFN from API
        generatedXMLFileName = await xmlUtility.generateXMLFileRTRForVG("RTRWithAPRA_VG.xml", apiRequestContext, SelectionOfMember.isMemberToSelectExsisting, SelectionOfMember.isTFNToBePassed);
    });
    ...
    await test.step("Verify Member rollover-in Details In Transactions Screen", async () => {
        ...
        if (SelectionOfMember.isMemberToSelectExsisting === true) {
            await memberPage.memberOverview();
        } else {
...
test("RTR is processed from SMSF with TFN -@wed", async ({ memberPage, superSteam, globalPage, apiRequestContext }) => {
    ...
    await test.step("Generate XML file for upload", async () => {
        ...
        //Here we have set first parameter as isMemberToSelectExsisting which is passing from JSON, so if we make it as true will select Exsisting Member or else if it will create a New Member from API and perform remaining process
        //Here we have set second parameter as isTFNToBePassed which is passing from JSON, so if we make it as false it will provide a TFN for the New Member created from API If we set to true it will create a new member without TFN from API
        generatedXMLFileName = await xmlUtility.generateXMLFileRTRForVG("RTRWithSMSF_VG.xml", apiRequestContext, SelectionOfMember.isMemberToSelectExsisting, SelectionOfMember.isTFNToBePassed);
    });
...
test("Member is processed with Partial Rollover-Out Exit with SMSF Fund -@IRR", async ({ memberPage, superSteam, globalPage, apiRequestContext }) => {
    ...
    await test.step("Generate XML file for upload", async () => {
        ...
        //Here we have set first parameter as isMemberToSelectExsisting which is passing from JSON, so if we make it as true will select Exsisting Member or else if it will create a New Member from API and perform remaining process
        //Here we have set second parameter as isTFNToBePassed which is passing from JSON, so if we make it as false it will provide a TFN for the New Member created from API If we set to true it will create a new member without TFN from API
        generatedXMLFileName = await xmlUtility.generateXMLFileIRRForVG("IRR_Partial-Exit_VG_SMSF.xml", apiRequestContext, SelectionOfMember.isMemberToSelectExsisting, SelectionOfMember.isTFNToBePassed);
    });
    ...
    await test.step("Verify member Initial Rollover-Out Request by Superstream", async () => {
        await new Promise((resolve) => setTimeout(resolve, 60000));
        await memberPage.verifySuperstreamProcessForVG("SuperStream - Rollover Out", ['Process step completed with note: Rollover received from other fund letter sent.']);
...
test("Member is processed with Full Rollover-Out Exit with SMSF Fund -@IRR", async ({ memberPage, superSteam, globalPage, apiRequestContext }) => {
    ...
    await test.step("Generate XML file for upload", async () => {
        ...
        //Here we have set first parameter as isMemberToSelectExsisting which is passing from JSON, so if we make it as true will select Exsisting Member or else if it will create a New Member from API and perform remaining process
        //Here we have set second parameter as isTFNToBePassed which is passing from JSON, so if we make it as false it will provide a TFN for the New Member created from API If we set to true it will create a new member without TFN from API
        generatedXMLFileName = await xmlUtility.generateXMLFileIRRForVG("IRR_Full-Exit_VG_SMSF.xml", apiRequestContext, SelectionOfMember.isMemberToSelectExsisting, SelectionOfMember.isTFNToBePassed);
    });
    ...
    await test.step("Verify member Initial Rollover-Out Request by Superstream", async () => {
        await new Promise((resolve) => setTimeout(resolve, 70000));
        await memberPage.verifySuperstreamProcessForVG("SuperStream - Rollover Out", ['Process retry should be executed.']);
    });
...
test("GCTR is processed by superstream-@GCTR", async ({ memberPage, superSteam, globalPage, apiRequestContext }) => {
    ...
    await test.step("Generate XML file for upload", async () => {
        ...
        generatedXMLFileName = await xmlUtility.generateXMLFileGCTRForVG("GCTR_VG.xml", apiRequestContext, SelectionOfMember.isMemberToSelectExsisting, SelectionOfMember.isTFNToBePassed);
    });
...
test("GCTAR is processed by superstream-@GCTAR", async ({ memberPage, superSteam, globalPage, apiRequestContext }) => {
    ...
    let generatedXMLFileName: string | { destinationFileName: string; conversationId: string, payrollNumber: string, member: string, surName: string, year: number, month: string, day: number };
    await test.step("Generate XML file for upload", async () => {
        ...
        generatedXMLFileName = await xmlUtility.generateXMLFileGCTARForVG("GCTAR_VG.xml", apiRequestContext, SelectionOfMember.isMemberToSelectExsisting, SelectionOfMember.isTFNToBePassed);
    });
