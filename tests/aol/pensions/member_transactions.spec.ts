...
test(
  fundName() + "-Manual Roll-in - Pension Member @pension",
  async ({
    navBar,
    ...
    await test.step("Add a New Pensions Member", async () => {
      await pensionTransactionPage.shellAccount(
        navBar,
        pensionAccountPage,
        apiRequestContext
      );
...
test(
  ...
  async ({
    ...
    await test.step("Investments and Balances Page", async () => {
      ...
      if (memberId) {
        const investments =
          await ShellAccountCreationApiHandler.getMemberInvestments(
            transactionApi,
            memberId!
          );
    ...
    await test.step("Validate the Payment Details In Transactions Screen", async () => {
      ...
      if (memberId) {
        ...
        const paymentDetails = await TransactionsApiHandler.fetchPaymentDetails(
          apiRequestContext,
    ...
    await test.step("Validate Member Status", async () => {
      const memberId = getMemberId();
      if (memberId) {
        const summary = await MemberApiHandler.fetchMemberSummary(
          apiRequestContext,
          memberId,
          false
        );
    ...
    await test.step("Validate MATS Report", async () => {
      const memberId = getMemberId();
      if (memberId) {
        const MAASReport = await ShellAccountCreationApiHandler.getMemberReport(
          transactionApi,
          memberId,
          "MATS Submit"
        );
...
test(
  ...
  async ({
    ...
    if (pensionMember.generate_test_data_from_api) {
      ...
      // Select Existing Pension Member
      const memberNo =
        pensionMember.members
          .ABP_Commutation_Rollover_And_UNP_Commutation_Partial_Member_Number;
      await test.step("Select Existing Pension Member", async () => {
        await navBar.selectMember(memberNo);
        const linearId = await MemberApiHandler.fetchMemberDetails(
          apiRequestContext,
...
test(
  ...
  async ({
    ...
    if (pensionMember.generate_test_data_from_api) {
      ...
      const memberNo =
        pensionMember.members
          .TTR_Commutation_Rollout_Partial_And_Full_Member_Number;
      await test.step("Select Existing Pension Member", async () => {
        await navBar.selectMember(memberNo);
        const linearId = await MemberApiHandler.fetchMemberDetails(
          apiRequestContext,
...
test(
  ...
  async ({
    ...
    if (pensionMember.generate_test_data_from_api) {
      ...
      // Select Existing Pension Member
      const memberNo =
        pensionMember.members.ABP_UNP_Commutation_Partial_Member_Number;
      await test.step("Select Existing Pension Member", async () => {
        await navBar.selectMember(memberNo);
        const linearId = await MemberApiHandler.fetchMemberDetails(
          apiRequestContext,
...
test(
  ...
  async ({
    ...
    if (pensionMember.generate_test_data_from_api) {
      ...
      // Select Existing Pension Member
      const memberNo =
        pensionMember.members.ABP_Commutation_Rollover_Full_Member_Number;
      await test.step("Select Existing Pension Member", async () => {
        await navBar.selectMember(memberNo);
        const linearId = await MemberApiHandler.fetchMemberDetails(
          apiRequestContext,
    ...
    await test.step("Validate Member Fee Details", async () => {
      ...
      if (memberId) {
        ...
        const feeDetails = await ShellAccountCreationApiHandler.getMemberFee(
          transactionApi,
          memberId
        );
...
test(
  ...
  async ({
    ...
    if (pensionMember.generate_test_data_from_api) {
      ...
      // Select Existing Pension Member
      const memberNo =
        pensionMember.members.ABP_Commutation_UNP_Full_Member_Number;
      await test.step("Select Existing Pension Member", async () => {
        await navBar.selectMember(memberNo);
        const linearId = await MemberApiHandler.fetchMemberDetails(
          apiRequestContext,
    ...
    await test.step("Validate Member Status", async () => {
      ...
      await test.step("Validate MATS Report", async () => {
        ...
        if (memberId) {
          const MAASReport =
            await ShellAccountCreationApiHandler.getMemberReport(
              transactionApi,
              memberId,
              "MATS Submit"
            );
...
test(
  fundName() + "-TTR RLO Commutation - Full Exit @pension",
  async ({
    navBar,
    pensionTransactionPage,
    pensionAccountPage,
  ...
  async ({
    ...
    try {
      ...
      if (pensionMember.generate_test_data_from_api) {
        ...
        await test.step("Select Existing Pension Member", async () => {
          await navBar.selectMember(memberNo);
          await MemberApiHandler.fetchMemberDetails(
            apiRequestContext,
            memberNo!
          );
          await globalPage.captureScreenshot("Pension Member Selection page");
...
test(
  ...
  async ({
    ...
    if (pensionMember.generate_test_data_from_api) {
      ...
      const memberNo =
        pensionMember.members
          .TTR_Lump_Sum_Unrestricted_Unreserved_Fund_Withdrawal_Error;
      await test.step("Select Existing Pension Member", async () => {
        await navBar.selectMember(memberNo);
        const linearId = await MemberApiHandler.fetchMemberDetails(
          apiRequestContext,
...
test(
  fundName() + "-ABP Pension commencement WITH PTB @pension",
  async ({
    navBar,
    pensionTransactionPage,
    pensionAccountPage,
    ...
    await navBar.navigateToPensionMembersPage();
    await pensionTransactionPage.ptbTransactions(
      navBar,
      pensionAccountPage,
      apiRequestContext
    );
...
test(
  fundName() +
    "Verify the updating of member's CRN in the account details @pension",
  async ({
    navBar,
    accountInfoPage,
    memberPage,
    ...
    if (pensionMember.generate_test_data_from_api) {
      ...
      await test.step("Create New Member Account", async () => {
        const { createMemberNo } = await memberPage.accumulationMember(
          navBar,
          accountInfoPage,
          apiRequestContext,
          internalTransferPage
