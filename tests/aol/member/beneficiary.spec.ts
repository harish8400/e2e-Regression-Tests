...
test(fundName()+"- Add new non binding nomination on an existing account with all portion matched to 100% @demorun", async ({ apiRequestContext, navBar, beneficiaryPage, memberPage ,accountInfoPage,internalTransferPage }) => {
...
test(fundName()+"-Add new Binding lapsing nomination on an existing account with all portion matched to 100% @demorun", async ({ internalTransferPage, apiRequestContext, memberPage, navBar, beneficiaryPage ,accountInfoPage }) => {
...
test(fundName()+"-Non binding or Binding lapsing nomination cannot be updated if total portion is not equal to 100% @demorun", async ({ internalTransferPage, apiRequestContext, memberPage, navBar, beneficiaryPage ,accountInfoPage }) => {
...
test(fundName() + "-Verify a new pension membership account creation, then alter the beneficiary details while membership is in Provisional status @demorun", async ({ navBar, beneficiaryPage, pensionTransactionPage, pensionAccountPage,  apiRequestContext }) => {
    try {
...
test(fundName() + "-Verify a new pension membership account creation, then alter the beneficiary details while membership is in Active status @demorun", async ({ navBar, beneficiaryPage, pensionTransactionPage, pensionAccountPage, apiRequestContext }) => {
    try {
        await test.step("Navigate to Pensions Members page", async () => {
            await navBar.navigateToPensionMembersPage();
        })
