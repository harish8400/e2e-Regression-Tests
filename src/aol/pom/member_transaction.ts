...
export class MemberTransactionsPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.relationshipEditBtn = page
      .locator("button")
      .filter({ hasText: "Edit Content" });
    ...
    this.verifyRolloutProcessSuccess = page.getByText(
      "Process step completed with note: Manual Super Stream rollout correspondence sen"
    );
  ...
  async memberRolloverIn(contributionType?: String, TFN?: Boolean) {
    ...
    if (TFN == true) {
      ...
      await this.reviewCase.approveAndVerifyError(
        this.memberContributionErrorMessage
      );
    }
  ...
  /** Check whether Admin fee transaction created for Pension - Commutation Rollout*/
